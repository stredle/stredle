...
#if defined(NDEBUG)
# error "Stredle cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xff3c710bf19a8d7d00b4436a8923016c542c58f5ab339cd612cbf00d52470561");
...
const string strMessageMagic = "Stredle Signed Message:\n";
...
{
    // Stredle: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Stredle: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Stredle: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc9b7c4559b9f02d04ddcaf6f80b9df35e600734e66930d9c041e4823527d78ee");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("4d267621c4944bbe03686c82023d22f8399179a7790baaa6166b1b19cb900e6a58a8066a074da15eab8b0eb5f1b94859b921884fdffcff1e431151dae3a7d0a762") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521658424;
        block.nBits    = 520503525;
        block.nNonce   = 2084537496;

        if (fTestNet)
        {
            block.nTime    = 1521658424;
            block.nNonce   = 385297657;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x05a545be21d52037236b2ef2e0aef2801054295601f8f16fbd2f1ecac73b440d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa0, 0xcf, 0xc5, 0xc0 }; // Stredle: +1 to BTC's value
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StredleMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StredleMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StredleMiner(CWallet *pwallet)
{
    printf("StredleMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stredle-miner");
    ...
    try { loop {
        ...
        printf("Running StredleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StredleMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StredleMiner, pwallet));
}
